...
const mask_helper = (_ => {
	...
	vm.float2Digitos = value => `${value}`
		.replace(/\D/g, "") //Remove tudo o que nao e digito
		.replace(/(\d)(\d{2})$/, "$1.$2") //Coloca ponto antes dos 2 ultimos digitos
	...
	vm.removeDiacriticos = str => {
		...
		for (let i=0; i < mapaDefault.length; i++){
		    let letrasDoMapa = mapaDefault[i].letras.split("");
		    for (let j=0; j < letrasDoMapa.length ; j++){
		        mapaDiacriticos[letrasDoMapa[j]] = mapaDefault[i].base;
		    }
		}

		const letrasDaPalavra = str.split("");
		let newStr = "";
		for(let indice = 0; indice < letrasDaPalavra.length; indice++) {
			const caractereAtual = letrasDaPalavra[indice]
		    newStr += caractereAtual in mapaDiacriticos ? mapaDiacriticos[caractereAtual] : caractereAtual;
		}
		return newStr;
	}
